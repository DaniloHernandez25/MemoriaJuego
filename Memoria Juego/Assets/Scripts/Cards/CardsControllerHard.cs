    ...
    private IEnumerator GuardarProgresoEnCSV()
    {
        ...
        for (int i = 1; i < lineas.Length; i++)
        {
            ...
            if (columnas[0] == nombre && columnas[1] == fecha)
            {
                ...
                if (!int.TryParse(columnas[2], out int fasesActuales))
                {
                    fasesActuales = 0;
                    Debug.LogWarning($"No se pudo parsear fases completadas, iniciando con 0");
                }
                ...
                if (nivelCompletado == nivelesActuales)
                {
                    ...
                    // 🔑 Aquí usamos esUltimoNivel
                    if (esUltimoNivel)
                    {
                        fasesActuales++;
                        columnas[2] = fasesActuales.ToString();
                        Debug.Log($"✅ Último nivel superado → fases completadas ahora = {fasesActuales}");
                    }
